...
def add_review(film_id):
    if request.method == 'POST':
        ...
        session = Session(engine)
        # Creating query
        clean_text = bleach.clean(form['text'])
        new_review = Reviews(film_id=film_id, user_id=current_user.id, score=form['score'], text=clean_text)
...
def new_film():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            session = Session(engine)
            cleaned_text = bleach.clean(form['description'])
            new_film = Films(name=form['name'], year=form['year'], country=form['country'], director=form['director'],
                             ...
                             description=cleaned_text, poster_id=poster_id)
